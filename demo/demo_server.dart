...
class DemoController extends BaseController {
  static final List<Map<String, dynamic>> _products = [
    ...
    {'id': '2', 'name': 'Coffee Mug', 'price': 12.50, 'category': 'home', 'stock': 50},
    {'id': '3', 'name': 'Wireless Mouse', 'price': 45.00, 'category': 'electronics', 'stock': 30},
    {'id': '4', 'name': 'Notebook', 'price': 8.99, 'category': 'office', 'stock': 100},
    {'id': '5', 'name': 'Smartphone', 'price': 699.99, 'category': 'electronics', 'stock': 25},
  ];
  ...
  Future<Response> searchProducts(Request request) async {
    ...
    final paginatedProducts = filteredProducts.skip(offsetInt).take(limitInt).toList();
  ...
  Future<Response> createProduct(Request request) async {
    ...
    try {
      ...
      if (!data.containsKey('name') || !data.containsKey('price') || !data.containsKey('category')) {
        final response = ApiResponse.error('Missing required fields: name, price, category');
        return jsonResponse(response.toJson(), statusCode: 400);
      }
      ...
      print('✅ Created product: ${newProduct['name']} (\$${newProduct['price']})');
  ...
  Future<Response> getStats(Request request) async {
    ...
    final totalValue = _products.fold(0.0, (sum, p) => sum + (p['price'] as double) * (p['stock'] as int));
    final categories = _products.map((p) => p['category']).toSet().toList();
    final avgPrice = _products.fold(0.0, (sum, p) => sum + (p['price'] as double)) / totalProducts;
    ...
    final response = ApiResponse.success(stats, 'Statistics generated successfully');
    return jsonResponse(response.toJson());
  }
