...
class EnhancedReflectionHelper extends ReflectionHelper {
  ...
  static bool get isReflectionAvailable =>
      ReflectionHelper.isReflectionAvailable;
  ...
  static List<String> getControllerMethods(Type controllerType) {
    ...
    try {
      ...
      Log.d(
        'Found ${methods.length} HTTP methods in $controllerType: ${methods.join(', ')}',
      );
      return methods;
    } catch (e) {
  ...
  static Future<List<Middleware>> createJWTValidationMiddleware(
    ...
    try {
      ...
      if (controllerValidators != null) {
        Log.i(
          '🔐 Controller-level JWT validation: $controllerType.$methodName',
        );
        middlewares.add(
          _createValidationMiddleware(
      ...
      Log.e(
        'Error creating JWT validation middleware for $controllerType.$methodName: $e',
      );
      return middlewares;
    }
  }
  ...
  static bool _hasJWTPublicAnnotation(Type controllerType, String methodName) {
    ...
    try {
      ...
      for (final declaration in classMirror.declarations.entries) {
        if (declaration.key == methodSymbol &&
            declaration.value is mirrors.MethodMirror) {
          final methodMirror = declaration.value as mirrors.MethodMirror;
          ...
          for (final metadata in methodMirror.metadata) {
            if (metadata.reflectee is JWTPublic) {
              Log.d(
                'Found @JWTPublic annotation on $controllerType.$cleanMethodName',
              );
              return true;
            }
          }
      ...
      Log.w(
        'Failed to check JWTPublic annotation for $controllerType.$methodName: $e',
      );
      return false;
    }
  }
  ...
  static JWTEndpointConfig? _getJWTEndpointValidators(
    Type controllerType,
    String methodName,
  ) {
    ...
    try {
        ...
        if (declaration.key == methodSymbol &&
          ...
          for (final metadata in methodMirror.metadata) {
            if (metadata.reflectee is JWTEndpoint) {
              ...
              Log.d(
                'Found @JWTEndpoint annotation on $controllerType.$cleanMethodName with ${annotation.validators.length} validators',
              );
              return JWTEndpointConfig(
      ...
      Log.w(
        'Failed to get JWTEndpoint validators for $controllerType.$methodName: $e',
      );
      return null;
    }
  }
    ...
    try {
      ...
      for (final metadata in classMirror.metadata) {
        if (metadata.reflectee is JWTController) {
          ...
          Log.d(
            'Found @JWTController annotation on $controllerType with ${annotation.validators.length} validators, requireAll: ${annotation.requireAll}',
          );
          return JWTControllerConfig(
            annotation.validators,
            annotation.requireAll,
          );
        }
        ...
        try {
          ...
          for (final validator in validators) {
            ...
            if (result.isSuccess) {
              ...
              final reason =
                  result.errorMessage ?? validator.defaultErrorMessage;
          ...
          if (requireAll) {
            ...
            isAuthorized = validationResults.every(
