...
class RouterBuilder {
  ...
  static Future<Router> buildFromController(Object controller) async {
    ...
    final reflectionRouter = await ReflectionHelper.buildRoutesWithReflection(
      controller,
    );
    ...
    Log.w(
      'Reflection not available. Please override the router getter manually.',
    );
    Log.w(
      'Example: @override Router get router { final r = Router(); r.get("/", handler); return r; }',
    );
  ...
  static String? extractControllerPath(Object controller) {
    try {
      ...
      final reflectionRouter = ReflectionHelper.extractControllerPath(
        controller,
      );
      return reflectionRouter;
    } catch (e) {
