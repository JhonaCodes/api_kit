...
class RouterBuilder {
  ...
  static Future<Router> _buildWithStaticAnalysis(BaseController controller) async {
    try {
      Log.i('🚀 Building routes using static analysis (AOT compatible)...');
      
      // Try static analysis first
      final staticRouter = await StaticRouterBuilder.buildFromController(controller);
  ...
  static String? extractControllerPath(Object controller) {
    ...
    Log.w('Controller path extraction from static analysis not yet implemented');
    return null;
  }
