...
class ControllerRegistry {
  ...
  static Future<List<BaseController>> discoverControllers([String? projectPath]) async {
    try {
      ...
      for (final controllerAnnotation in restControllers) {
        try {
          ...
          Log.w('Failed to instantiate controller ${controllerAnnotation.targetName}: $e');
        }
      }
      
      _isDiscovered = true;
      Log.i('🎯 Auto-discovered ${controllers.length} controllers successfully');
      
      return controllers;
      
    } catch (e, stackTrace) {
      Log.e('Error during controller auto-discovery', error: e, stackTrace: stackTrace);
      return [];
    }
  }
  ...
  static Future<BaseController?> _instantiateController(String className) async {
    try {
      ...
      for (final library in mirrors.libraries.values) {
        for (final classMirror in library.declarations.values) {
          if (classMirror is ClassMirror && 
              classMirror.simpleName.toString().replaceAll('Symbol("', '').replaceAll('")', '') == className) {
  ...
  static bool _extendsBaseController(ClassMirror classMirror) {
    ...
    while (current != null) {
      final className = current.simpleName.toString().replaceAll('Symbol("', '').replaceAll('")', '');
