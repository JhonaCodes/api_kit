...
class AnnotationVisitor extends RecursiveAstVisitor<void>{
  ...
  void visitAnnotation(Annotation node) {
    if (verbose) {
      ...
      print('DEBUG: Library ID: ${node.elementAnnotation?.element?.library?.identifier}');
    ...
    if (node.isFromMyPackage) {
      ...
      if (context != null) {
        ...
        if (verbose) {
          print('DEBUG: Adding annotation ${node.annotationType} for ${context.name}');
        }
        
        foundAnnotations.add(AnnotationDetails.create(
  ...
  _Context? _getCurrentContext() {
    if (_currentFunction != null) {
      return _Context(_currentFunction!.name.lexeme, AnnotationTargetType.topLevelFunction);
    }
    if (_currentClass != null && _currentMethod != null) {
      return _Context('${_currentClass!.name.lexeme}.${_currentMethod!.name.lexeme}', AnnotationTargetType.instanceMethod);
    }
