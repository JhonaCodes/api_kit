  ...
  group('JWT Comprehensive Tests - ALL Use Cases', () {
    ...
    setUpAll(() async {
      ...
      server.configureJWTAuth(
        jwtSecret: 'comprehensive-test-secret-key-minimum-32-characters-for-jwt-validation',
      ...
      result.when(
        ...
        err: (error) => throw Exception('Failed to start comprehensive test server: ${error.msm}'),
      );
    });
    ...
    group('1. PUBLIC ENDPOINTS (@JWTPublic)', () {
      test('1.1 Should allow access to @JWTPublic endpoints without any JWT', () async {
      ...
      test('1.2 Should allow @JWTPublic even with invalid JWT in header', () async {
        final response = await _makeRequest('GET', '/api/public/open', 
      ...
      test('1.3 Should prioritize @JWTPublic over controller-level validation', () async {
        // Finance controller has validation but this endpoint is public
        final response = await _makeRequest('GET', '/api/finance/public-balance');
    ...
    group('2. CONTROLLER-LEVEL VALIDATION (@JWTController)', () {
      test('2.1 Should deny access without JWT to @JWTController protected endpoint', () async {
      ...
      test('2.5 Should deny access with valid JWT but insufficient permissions', () async {
    ...
    group('3. ENDPOINT-LEVEL OVERRIDE (@JWTEndpoint)', () {
      test('3.1 Should override controller validation with endpoint-specific validation', () async {
        ...
        final response = await _makeRequest('POST', '/api/finance/high-value-transaction',
            headers: {'Authorization': 'Bearer $financialToken', 'Content-Type': 'application/json'},
      ...
      test('3.2 Should deny endpoint access when custom validation fails', () async {
        ...
        final response = await _makeRequest('POST', '/api/finance/high-value-transaction',
            headers: {'Authorization': 'Bearer $lowClearanceToken', 'Content-Type': 'application/json'},
            body: jsonEncode({'amount': 25000}));
        
        expect(response.statusCode, equals(403));
      });
      
      test('3.3 Should allow admin-only endpoint with proper admin token', () async {
        final adminToken = _createValidAdminToken();
        
        final response = await _makeRequest('DELETE', '/api/support/tickets/123',
            headers: {'Authorization': 'Bearer $adminToken'});
    ...
    group('4. AND LOGIC VALIDATION (requireAll: true)', () {
      test('4.1 Should require ALL validators to pass with requireAll: true', () async {
        ...
        final response = await _makeRequest('GET', '/api/admin/sensitive-data',
            headers: {'Authorization': 'Bearer $adminToken'});
        
        expect(response.statusCode, equals(200));
      });
      
      test('4.2 Should fail if ANY validator fails with requireAll: true', () async {
    ...
    group('5. OR LOGIC VALIDATION (requireAll: false)', () {
      test('5.1 Should pass if admin requirement is met (OR logic)', () async {
        ...
        final response = await _makeRequest('GET', '/api/support/tickets',
            headers: {'Authorization': 'Bearer $adminToken'});
      ...
      test('5.2 Should pass if department requirement is met (OR logic)', () async {
        final supportToken = _createJWT({
          'user_id': 'support123',
          'role': 'user',
          'department': 'support',
      ...
      test('5.3 Should fail if NONE of the validators pass (OR logic)', () async {
    ...
    group('6. JWT PAYLOAD ACCESS IN CONTEXT', () {
      test('6.1 Should provide full JWT payload in request context', () async {
        ...
        final response = await _makeRequest('GET', '/api/admin/profile',
    ...
    group('7. TOKEN BLACKLIST FUNCTIONALITY', () {
      test('7.1 Should deny access with blacklisted token', () async {
        ...
        final response1 = await _makeRequest('GET', '/api/admin/users',
            headers: {'Authorization': 'Bearer $adminToken'});
        ...
        final response2 = await _makeRequest('GET', '/api/admin/users',
            headers: {'Authorization': 'Bearer $adminToken'});
      ...
      test('7.2 Should allow access after token is removed from blacklist', () async {
        final adminToken = _createValidAdminToken();
    ...
    group('8. EDGE CASES AND ERROR SCENARIOS', () {
      test('8.1 Should handle missing Authorization header gracefully', () async {
        final response = await _makeRequest('GET', '/api/admin/users');
      ...
      test('8.6 Should handle future-dated JWT (iat in future)', () async {
        final futureToken = _createJWT({
          ...
          'active': true,
          'permissions': ['admin_access'],
          'iat': DateTime.now().add(Duration(hours: 1)).millisecondsSinceEpoch ~/ 1000, // Future
          'exp': DateTime.now().add(Duration(hours: 2)).millisecondsSinceEpoch ~/ 1000,
        });
    ...
    group('9. COMPLEX VALIDATION SCENARIOS', () {
      test('9.1 Should handle multiple department validator', () async {
        ...
        final response = await _makeRequest('GET', '/api/mixed/department-test',
      ...
      test('9.2 Should handle financial validator with edge amounts', () async {
        ...
        final response = await _makeRequest('POST', '/api/mixed/financial-edge-case',
            headers: {'Authorization': 'Bearer $financialToken', 'Content-Type': 'application/json'},
      ...
      test('9.3 Should handle business hours validation edge cases', () async {
        ...
        final response = await _makeRequest('GET', '/api/edge/business-hours-test',
    ...
    group('10. PERFORMANCE AND SCALABILITY', () {
      test('10.1 Should handle rapid consecutive requests', () async {
        ...
        for (int i = 0; i < 10; i++) {
          futures.add(_makeRequest('GET', '/api/admin/users',
              headers: {'Authorization': 'Bearer $adminToken'}));
        }
      ...
      test('10.2 Should handle different tokens simultaneously', () async {
        ...
        final futures = await Future.wait([
          _makeRequest('GET', '/api/admin/users',
              headers: {'Authorization': 'Bearer $adminToken'}),
          _makeRequest('GET', '/api/support/tickets',
              headers: {'Authorization': 'Bearer $supportToken'}),
        ]);
    ...
    group('11. JWT CONFIGURATION MANAGEMENT', () {
      test('11.1 Should handle JWT configuration changes', () async {
        final originalCount = server.blacklistedTokensCount;
        ...
        server.configureJWTAuth(
          jwtSecret: 'new-test-secret-key-for-configuration-testing-minimum-32-chars',
...
Future<TestResponse> _makeRequest(String method, String path, 
...
@JWTController([
  ...
  const MyBusinessHoursValidator(startHour: 8, endHour: 18),
], requireAll: true)
...
@JWTController([
  const MyDepartmentValidator(allowedDepartments: ['finance', 'accounting']),
])
...
@JWTController([
  const MyAdminValidator(),
  const MyDepartmentValidator(allowedDepartments: ['support', 'customer_service']),
...
class SupportControllerTest extends BaseController {
  ...
  @DELETE('/tickets/<id>')
  @JWTEndpoint([
    const MyAdminValidator(), // Only admin can delete
  ])
...
@JWTController([
  const MyDepartmentValidator(allowedDepartments: ['general']),
])
class MixedValidationControllerTest extends BaseController {
  
  @GET('/department-test')
  @JWTEndpoint([
    const MyDepartmentValidator(allowedDepartments: ['finance', 'admin'], requireManagerLevel: true),
  ])
  Future<Response> departmentTest(Request request) async {
    return jsonResponse(jsonEncode({
      'success': true,
      'message': 'Department validation passed',
    }));
  }
...
class EdgeCaseControllerTest extends BaseController {
  ...
  @JWTEndpoint([
    const MyBusinessHoursValidator(startHour: 0, endHour: 23), // Almost 24/7
  ])
