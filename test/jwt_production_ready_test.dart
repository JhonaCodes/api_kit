  ...
  group('JWT Production Ready Tests - All Use Cases Validated', () {
    ...
    setUpAll(() async {
      ...
      server.configureJWTAuth(
        jwtSecret: 'production-test-secret-key-minimum-32-characters-for-complete-validation',
      ...
      result.when(
        ...
        err: (error) => throw Exception('Failed to start production test server: ${error.msm}'),
      );
    });
    ...
    group('CRÍTICO: Endpoints Públicos (@JWTPublic)', () {
      ...
      test('✅ @JWTPublic debe tener prioridad sobre validación de controller', () async {
      ...
      test('✅ @JWTPublic debe funcionar incluso con JWT inválido en header', () async {
        final response = await _request('GET', '/api/public/info',
    ...
    group('CRÍTICO: Override Endpoint (@JWTEndpoint)', () {
      test('✅ Debe usar validación específica de endpoint', () async {
        ...
        final response = await _request('POST', '/api/finance/transaction',
            headers: {
      ...
      test('❌ Debe fallar validación específica cuando no cumple requisitos', () async {
      ...
      test('✅ Admin-only endpoint debe funcionar con token admin', () async {
        ...
        final response = await _request('DELETE', '/api/support/ticket/123',
            headers: {'Authorization': 'Bearer $adminToken'});
    ...
    group('CRÍTICO: Lógica AND (requireAll: true)', () {
      test('✅ Debe requerir TODOS los validadores pasen', () async {
        ...
        final response = await _request('GET', '/api/admin/critical',
    ...
    group('CRÍTICO: Acceso a JWT Payload en Context', () {
      test('✅ Debe proveer payload completo en request context', () async {
        ...
        final response = await _request('GET', '/api/admin/profile',
        ...
        expect(data['jwt_payload']['custom_field'], equals('valor_personalizado'));
        expect(data['context_shortcuts']['user_id'], equals('admin456'));
        expect(data['context_shortcuts']['user_email'], equals('admin@test.com'));
    ...
    group('CRÍTICO: Token Blacklist', () {
      test('✅ Debe denegar acceso con token blacklisteado', () async {
        ...
        final response1 = await _request('GET', '/api/admin/users',
            headers: {'Authorization': 'Bearer $adminToken'});
        ...
        final response2 = await _request('GET', '/api/admin/users',
            headers: {'Authorization': 'Bearer $adminToken'});
    ...
    group('CRÍTICO: Casos Edge y Errores', () {
      ...
      test('❌ JWT con iat en el futuro', () async {
        final futureToken = _createTestJWT({
          ...
          'active': true,
          'permissions': ['admin_access'],
          'iat': DateTime.now().add(Duration(hours: 1)).millisecondsSinceEpoch ~/ 1000,
    ...
    group('CRÍTICO: Performance y Escalabilidad', () {
      test('✅ Debe manejar múltiples requests concurrentes', () async {
        ...
        for (int i = 0; i < 5; i++) {
          futures.add(_request('GET', '/api/admin/users',
              headers: {'Authorization': 'Bearer $adminToken'}));
        }
      ...
      test('✅ Debe manejar diferentes tokens simultáneamente', () async {
        ...
        final responses = await Future.wait([
          _request('GET', '/api/admin/users',
              headers: {'Authorization': 'Bearer $adminToken'}),
          _request('GET', '/api/support/tickets',
              headers: {'Authorization': 'Bearer $supportToken'}),
        ]);
    ...
    group('CRÍTICO: Configuración JWT', () {
      test('✅ Debe manejar cambios de configuración JWT', () async {
        ...
        server.configureJWTAuth(
          jwtSecret: 'nueva-clave-secreta-para-pruebas-de-configuracion-minimo-32-caracteres',
...
Future<TestResponse> _request(String method, String path, 
...
@JWTController([
  ...
  const MyBusinessHoursValidator(startHour: 0, endHour: 23), // 24/7 para tests
], requireAll: true)
...
class MixedController extends BaseController {
  ...
  @GET('/test')
  @JWTEndpoint([
    const MyDepartmentValidator(allowedDepartments: ['any']),
  ])
  Future<Response> testEndpoint(Request request) async {
    return jsonResponse(jsonEncode({
      'success': true,
      'mixed_validation': true,
    }));
  }
}