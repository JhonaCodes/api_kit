...
void main() {
  group('Static Analysis Migration Tests', () {
    ...
    test('should validate HTTP method annotation data', () async {
      ...
      final getHello = result.ofType('Get')
      ...
      final createUser = result.ofType('Post')
          .where((p) => p.targetName.contains('TestControllerStatic.createUser'))
          .toList();
      ...
      expect(createUser.first.postInfo!.description, equals('Create a new user'));
    });
    
    test('should register methods in dispatcher', () async {
      expect(MethodDispatcher.isMethodRegistered('TestControllerStatic', 'getHello'), isTrue);
      expect(MethodDispatcher.isMethodRegistered('TestControllerStatic', 'createUser'), isTrue);
      expect(MethodDispatcher.isMethodRegistered('TestControllerStatic', 'updateUser'), isTrue);
      expect(MethodDispatcher.isMethodRegistered('TestControllerStatic', 'deleteUser'), isTrue);
      
      final registeredMethods = MethodDispatcher.getRegisteredMethods('TestControllerStatic');
    ...
    test('should use static analysis in RouterBuilder', () async {
      ...
      final routingMethod = await RouterBuilder.getRoutingMethod(testController);
    ...
    test('should validate annotation path combinations', () async {
      ...
      final controller = result.ofType('RestController')
      ...
      final getUser = result.ofType('Get')
  ...
  group('Non-BaseController Tests', () {
    test('should handle non-BaseController objects gracefully', () async {
      ...
      final routingMethod = await RouterBuilder.getRoutingMethod(nonBaseController);
  ...
  group('Performance Tests', () {
    ...
    test('should handle method dispatch efficiently', () async {
      ...
      for (int i = 0; i < 100; i++) {
        await MethodDispatcher.callMethod('TestControllerStatic', 'getHello', request);
      }
