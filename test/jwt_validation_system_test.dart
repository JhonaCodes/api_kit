...
void main() {
  group('JWT Validation System Tests', () {
    ...
    group('@JWTEndpoint Override Tests', () {
      test(
        'should use endpoint-specific validation that overrides controller',
        () async {
          final client = HttpClient();
          ...
          try {
            final request = await client.postUrl(
              Uri.parse(
                'http://localhost:$testPort/api/mixed/financial-operation',
              ),
            );
      ...
      test(
        'should deny endpoint access when custom validation fails',
        () async {
          final client = HttpClient();
...
@JWTController([TestAdminValidator()], requireAll: true)
...
@JWTController([
  TestDepartmentValidator(allowedDepartments: ['general']),
])
class MixedValidationController extends BaseController {
  @GET('/general')
  Future<Response> getGeneralData(Request request) async {
    return jsonResponse(
      jsonEncode({'success': true, 'message': 'General data accessed'}),
    );
  }

  @POST('/financial-operation')
  @JWTEndpoint([TestFinancialValidator(minimumAmount: 10000)])
...
class TestFinancialValidator extends JWTValidatorBase {
  ...
  ValidationResult validate(Request request, Map<String, dynamic> jwtPayload) {
    ...
    if (department != 'finance') {
      return ValidationResult.invalid(
        'Access restricted to finance department',
      );
    }
